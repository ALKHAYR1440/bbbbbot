...
class Builder:
    ...
    async def start(self):
        ...
        async def delete_bot(event):
            ...
            if ok:
                await event.respond("✅ تم حذف البوت من القائمة")
            else:
        ...
        async def ask_bot_token(event):
            await event.respond("أرسل توكن البوت من @BotFather 🤖")
            who = event.sender_id
        ...
        async def run_new_bot(event):
            ...
            if state == State.WAIT_FOR_BOT_TOKEN:
                ...
                await event.respond("🤖 جاري التحقق من التوكن")
                t = event.raw_text
                tokens = load_bots_tokens(tokens_json)
                if t in tokens:
                    await event.respond("❌ هذا التوكن موجود وهذا بوته : @" + usernameByToken(t) )
                    self.clean()
                    return
                try:
                    ...
                    print(e)
                    await event.respond("❌ هذا التوكن لا يعمل")
                    self.clean()
                    return
                ...
                await event.respond("  ✅ تم تشغيل البوت واضافته لقائمة البوتات واضافتك أدمن للبوت : @" + u.username + "\n اذهب للبوت وأرسل /Go" )
                self.clean()
                await task
